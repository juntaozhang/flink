...
public class TimestampsAndWatermarksOperator<T> extends AbstractStreamOperator<T>
    ...
    private transient WatermarkEmitter wmOutput;
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        wmOutput.initializeState(context);
    }
    ...
    public static final class WatermarkEmitter implements WatermarkOutput {
        ...
        public void initializeState(StateInitializationContext context) throws Exception {
            ...
            restoreWatermarks = operatorStateStore.getUnionListState(
                    new ListStateDescriptor<>(WATERMARK_STATE_NAME, BasicTypeInfo.LONG_TYPE_INFO));
            List<Long> watermarks = Lists.newArrayList(restoreWatermarks.get().iterator());
            if (context.isRestored() && !watermarks.isEmpty()) {
                currentWatermark = watermarks.stream().max(Long::compareTo).get();
                LOG.info("Find restore state for currentWatermark={}.", currentWatermark);
                output.emitWatermark(
                        new org.apache.flink.streaming.api.watermark.Watermark(currentWatermark));
            } else {
        ...
        public void emitWatermark(Watermark watermark) {
            ...
            LOG.info("emitWatermark: {}", watermark);
